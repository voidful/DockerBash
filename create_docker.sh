...
sudo docker exec -ti $USER_NAME sh -c "sudo apt-get update && sudo apt-get upgrade && sudo apt-get install -y openssh-server"

sudo docker exec -ti $USER_NAME sh -c "sudo useradd -m $USER_NAME -s /bin/bash;
